...
_MAX_REPLY_EXPAND_ATTEMPTS = 1000
_REPLY_CLICK_RETRIES = 3
...
def get_feed_detail(
    ...
    # 加载一级评论。完整子回复模式必须先完成这一阶段，不能在拿到固定数量的
    # 一级评论后直接返回。
    if load_all_comments or config.load_all_replies:
        try:
            _load_all_comments(page, config)
        except Exception as e:
            logger.warning("加载全部评论失败: %s", e)

    if config.load_all_replies:
        expand_status = _expand_all_replies(page, feed_id)
        # 展开操作会异步更新 __INITIAL_STATE__，重新读取结果而非复用展开前快照。
        response = _extract_feed_detail(page, feed_id)
        response.comment_load_status = _get_reply_progress(page, feed_id)
        response.comment_load_status["clickedButtons"] = expand_status.get("clickedButtons", 0)
        response.comment_load_status["failedButtons"] = expand_status.get("failedButtons", 0)
        response.comment_load_status["complete"] = (
            response.comment_load_status["remainingButtons"] == 0
            and response.comment_load_status["loadedReplies"]
            >= response.comment_load_status["declaredReplies"]
        )
    else:
        response = _extract_feed_detail(page, feed_id)
...
def _load_all_comments(page: Page, config: CommentLoadConfig) -> None:
    ...
    for attempt in range(max_attempts):
        ...
        # 定期点击展开按钮
        # 完整模式在一级评论加载完成后再统一展开；否则像过去一样，每隔几轮
        # 尝试展开小数量回复，避免在滚动阶段反复折叠/重排 DOM。
        if (
            config.click_more_replies
            and not config.load_all_replies
            and attempt % BUTTON_CLICK_INTERVAL == 0
        ):
...
def _get_reply_progress(page: Page, feed_id: str) -> dict:
    """读取页面状态中的子回复计数和待展开按钮数。"""
    feed_id_json = json.dumps(feed_id)
    selector_json = json.dumps(SHOW_MORE_BUTTON)
    result = page.evaluate(
        f"""
        (() => {{
            const map = window.__INITIAL_STATE__?.note?.noteDetailMap || {{}};
            const list = map[{feed_id_json}]?.comments?.list || [];
            let declaredReplies = 0;
            let loadedReplies = 0;
            for (const comment of list) {{
                declaredReplies += Number(comment.subCommentCount || 0) || 0;
                loadedReplies += Array.isArray(comment.subComments) ? comment.subComments.length : 0;
            }}
            return {{
                declaredReplies,
                loadedReplies,
                remainingButtons: document.querySelectorAll({selector_json}).length,
            }};
        }})()
        """
    )
    if not isinstance(result, dict):
        return {"declaredReplies": 0, "loadedReplies": 0, "remainingButtons": 0}
    return {
        "declaredReplies": int(result.get("declaredReplies", 0) or 0),
        "loadedReplies": int(result.get("loadedReplies", 0) or 0),
        "remainingButtons": int(result.get("remainingButtons", 0) or 0),
    }


def _expand_all_replies(page: Page, feed_id: str) -> dict:
    """逐个展开所有子回复，并在每次点击后验证页面状态确有增长。"""
    total_clicks = 0
    failed_buttons = 0

    for _ in range(_MAX_REPLY_EXPAND_ATTEMPTS):
        before = _get_reply_progress(page, feed_id)
        if before["remainingButtons"] == 0:
            before.update({"clickedButtons": total_clicks, "failedButtons": failed_buttons})
            return before

        progressed = False
        for retry in range(_REPLY_CLICK_RETRIES):
            try:
                # background.js 已实现真实 CDP 鼠标点击；这里不再使用 el.click()。
                page.click_nth_element(SHOW_MORE_BUTTON, 0)
            except Exception as e:
                logger.debug("点击展开回复失败（第 %d 次）：%s", retry + 1, e)
                continue
            ...
            while time.monotonic() < deadline:
                time.sleep(0.25)
                after = _get_reply_progress(page, feed_id)
                if (
                    after["loadedReplies"] > before["loadedReplies"]
                    or after["remainingButtons"] < before["remainingButtons"]
                ):
                    progressed = True
                    break
            if progressed:
        ...
        if not progressed:
            failed_buttons += 1
            logger.warning("展开子回复未生效，停止以避免无限点击：%s", before)
            before.update({"clickedButtons": total_clicks, "failedButtons": failed_buttons})
            return before

    progress = _get_reply_progress(page, feed_id)
    progress.update({"clickedButtons": total_clicks, "failedButtons": failed_buttons + 1})
    logger.warning("展开子回复达到安全上限：%s", progress)
    return progress
