...
class FeedDetailResponse:
    ...
    # 仅在请求展开子回复时返回。显式标注采集是否完整，避免把部分展开
    # 的回复误报为“全部回复”。
    comment_load_status: dict = field(default_factory=dict)
    ...
    def from_dict(cls, d: dict) -> FeedDetailResponse:
        return cls(
            ...
            comment_load_status=d.get("commentLoadStatus", {}),
        )
    ...
    def to_dict(self) -> dict:
        ...
        if self.comment_load_status:
            result["commentLoadStatus"] = self.comment_load_status
        return result
